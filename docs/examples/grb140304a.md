...
The paper models the injection as a violent shell collision ($E \propto t^{3.8}$) between 0.15 and 0.26 d. In blastwave, we approximate this using the magnetar spin-down model:
...
The standard wind density is $n(r) = A_* \times 3 \times 10^{35} / r^2$ cm$^{-3}$. In blastwave, $n(r) = n_\mathrm{wind} \cdot (r / 10^{17}\,\mathrm{cm})^{-2}$, so:
...
- The Laskar et al. model invokes a **shell collision** (discrete energy injection), which differs from the continuous magnetar spin-down model implemented in blastwave. The magnetar model produces a smoother energy injection profile.
